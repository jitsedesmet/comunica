...
async function depInfo(pckg) {
    const folders = (0, node_fs_1.readdirSync)(pckg.dir, { withFileTypes: true });
    const { files } = JSON.parse((0, node_fs_1.readFileSync)((0, node_path_1.join)(pckg.dir, 'package.json'), 'utf8'));
    let ignore;
    let checkedDeps;
    if (pckg.relativeDir.startsWith('engines/')) {
        ...
        checkedDeps = await checkDeps(pckg.dir, { ignorePath: 'falsePath', ignorePatterns: ignore }, (val) => val);
        ensureDependency({
            checkedDeps,
            dependency: '@comunica/runner',
            dependant: (0, node_path_1.join)(pckg.dir, 'engine-default.js'),
        });
        ensureDependency({
            checkedDeps,
        ...
        ignore = files ? folders.filter(elem => files.every((file) => !file.startsWith(elem.name))) : folders;
        ignore = ignore.map(x => x.isDirectory() ? `${x.name}/**` : x.name);
        checkedDeps = await checkDeps(pckg.dir, { ignorePatterns: ignore }, (val) => val);
    }
    let { dependencies, devDependencies, missing, using } = checkedDeps;
    dependencies = filterTypeUsedTypeDependencies({ dependencies, using });
    if (Object.values(using).flat().some(file => (0, node_fs_1.readFileSync)(file, 'utf8').toString().includes('require(\'process/\')') ||
        (0, node_fs_1.readFileSync)(file, 'utf8').toString().includes('require("process/")'))) {
...
async function depcheckTask() {
    const packages = (await getPackages(process.cwd())).packages.filter((pckg) => pckg.relativeDir.startsWith('packages/') ||
